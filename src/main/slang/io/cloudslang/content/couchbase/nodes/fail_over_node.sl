...
########################################################################################################################
#!!
#! @description: This operation fails over a specific node node and indicates that the node is no longer available in
#!               a cluster. That means replicated data on another node should be made available to clients.
#!               'https://developer.couchbase.com/documentation/server/4.6/rest-api/rest-node-failover.html'
#!
...
#!                    Default: 'true'
#!                    Optional
#! @input internal_node_ip_address: Indicates a specific node to be failed over.
...
#!
#! @result SUCCESS: Fail over the specified node completed successfully.
#! @result FAILURE: An error occurred while trying to fail over the specified nodes.
#!!#
########################################################################################################################
...
operation: 
  ...
  java_action: 
    ...
    class_name: 'io.io.cloudslang.content.couchbase.actions.nodes.FailOverNode'
    method_name: 'execute'
