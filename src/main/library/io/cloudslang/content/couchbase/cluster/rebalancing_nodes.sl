...
########################################################################################################################
#!!
#! @description: This operation starts rebalancing process on all cluster nodes.
#!               Note: The inputs: ejectedNodes, knownNodes cannot be both blank
#!               in order to successfully trigger nodes rebalancing.
#!
...
#!                    Default: 'true'
#!                    Optional
#! @input ejected_nodes: A string that contains: none, one or more nodes (that previously were part
...
#!                       Optional
#! @input known_nodes: A string that contains: none, one or more nodes that are known within the cluster.
#!                     Example: "ns_2@10.0.0.2,ns_2@10.0.0.3"
#!                     Optional
#! @input delimiter: A delimiter to used to separate multiple nodes.
#!                   Default: ','
#!                   Optional
#!
...
#!
#! @result SUCCESS: Rebalancing the cluster nodes completed successfully.
#! @result FAILURE: An error occurred while trying to rebalance the cluster nodes.
#!!#
########################################################################################################################
...
operation: 
  ...
  inputs: 
    ...
    - ejected_nodes:  
        required: false  
    - ejectedNodes: 
        default: ${get('ejected_nodes', '')}  
        required: false 
        private: true 
    - known_nodes:  
        required: false  
    - knownNodes: 
        default: ${get('known_nodes', '')}  
        required: false 
        private: true 
    - delimiter:
        default: ','
