...
public class FailOverNode {
    /**
     * Fails over a specific node node and indicates that the node is no longer available in a cluster. That means replicated
     * data on another node should be made available to clients.
     ...
     *                              Valid values: "true", "false"
     *                              Default value: "true"
     * @param internalNodeIpAddress Indicates a specific node to be failed over.
     *                              Example: "ns_2@10.0.0.2"
     * @return A map with strings as keys and strings as values that contains: outcome of the action (or failure message
     * and the exception if there is one), returnCode of the operation and the ID of the request
     */
    @Action(name = "Fail Over Node",
            outputs = {
                    @Output(RETURN_CODE),
